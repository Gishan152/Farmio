...
// Mock data for warehouses owned by the current user
const mockWarehouses = [
    ...
    },
    {
        ...
        analytics: {
            ...
            inventory: {
                totalReceived: 1800,
                totalDispatched: 1600,
                currentStock: 200,
                perishableItems: 120,
                nonPerishableItems: 80
            },
            financial: {
                storageRevenue: 70000,
            ...
            trends: [
                { month: 'Jun', received: 120, dispatched: 110, revenue: 12000 },
                { month: 'Jul', received: 150, dispatched: 140, revenue: 15000 },
                { month: 'Aug', received: 130, dispatched: 125, revenue: 13500 },
                { month: 'Sep', received: 170, dispatched: 160, revenue: 17000 },
                { month: 'Oct', received: 160, dispatched: 150, revenue: 15500 },
                { month: 'Nov', received: 180, dispatched: 170, revenue: 18000 },
                { month: 'Dec', received: 140, dispatched: 135, revenue: 14000 }
            ]
        }
    }
];
...
const ReportsAnalytics = () => {
    ...
    const currentUser = { name: "Kithmini", profilePic: "/alex.jpg" };

    useEffect(() => {
        // Simulate loading
        setTimeout(() => {
            setWarehouses(mockWarehouses);
            setSelectedWarehouseId(mockWarehouses[0].id);
            setLoading(false);
        }, 500);
    }, []);

    const selectedWarehouse = warehouses.find(w => w.id === Number(selectedWarehouseId));
    const reportData = selectedWarehouse ? selectedWarehouse.analytics : {};

    const generateReport = (type) => {
        if (!selectedWarehouse) return;
        const reportContent = `Warehouse Report - ${type.toUpperCase()}\nWarehouse: ${selectedWarehouse.name}\nOwner: ${currentUser.name}\nGenerated: ${new Date().toLocaleDateString()}\nPeriod: Last ${selectedPeriod} days\n\n${JSON.stringify(reportData[type], null, 2)}`;
        const blob = new Blob([reportContent], { type: 'text/plain' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `warehouse-${selectedWarehouse.name.toLowerCase().replace(/\s/g, '-')}-${type}-report-${new Date().toISOString().split('T')[0]}.txt`;
        a.click();
        ...
        <div className="min-h-screen bg-white-50 flex">
            ...
            <div className="fixed top-0 left-60 right-0 z-20">
                <PageHeader
                    ...
                    subtitle="View analytics and generate reports for each of your warehouses."
                    user={currentUser}
                />
            </div>
            {/* Main Content */}
            <main className="flex-1 lg:ml-60 pt-[72px] p-6 lg:p-8 transition-all duration-300 mt-20">
                <div className="max-w-7xl mx-auto">
                    {/* Warehouse Selector & Controls */}
                    <div className="mb-8 bg-white p-6 rounded-lg shadow flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    {selectedReport === 'overview' && (
                        <div>
                            <h2 className="text-xl font-semibold text-gray-800 mb-6">Business Overview - {selectedWarehouse?.name}</h2>
                    ...
                    {selectedReport === 'inventory' && (
                        <div>
                            <h2 className="text-xl font-semibold text-gray-800 mb-6">Inventory Analysis - {selectedWarehouse?.name}</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    {selectedReport === 'financial' && (
                        <div>
                            <h2 className="text-xl font-semibold text-gray-800 mb-6">Financial Performance - {selectedWarehouse?.name}</h2>
                    ...
                    {selectedReport === 'trends' && (
                        <div>
                            <h2 className="text-xl font-semibold text-gray-800 mb-6">Trends Analysis - {selectedWarehouse?.name}</h2>
                            <div className="bg-white p-6 rounded-lg shadow">
